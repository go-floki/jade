...
func (s *scanner) Next() *token {
	...
	case scnEOF:
		if outdent := s.indentStack.Back(); outdent != nil {
			...
			return &token{tokOutdent, "", nil, nil}
		}
...
func (s *scanner) NextRaw() *token {
	...
	for {
		...
		switch s.state {
		case scnEOF:
			return &token{tokText, result, map[string]string{"Mode": "raw"}, nil}
		case scnNewLine:
			s.state = scnLine
			...
			if tok := s.scanIndent(); tok != nil {
				...
				if level < 0 {
					s.stash.PushBack(&token{tokOutdent, "", nil, nil})
...
func (s *scanner) scanDoctype() *token {
	if sm := rgxDoctype.FindStringSubmatch(s.buffer); len(sm) != 0 {
		...
		s.consume(len(sm[0]))
		return &token{tokDoctype, sm[2], nil, nil}
	}
...
func (s *scanner) scanCondition() *token {
	if sm := rgxIf.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokIf, sm[1], nil, nil}
	}
...
func (s *scanner) scanEach() *token {
	if sm := rgxEach.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokEach, sm[3], map[string]string{"X": sm[1], "Y": sm[2]}, nil}
	}
...
func (s *scanner) scanAssignment() *token {
	if sm := rgxAssignment.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokAssignment, sm[2], map[string]string{"X": sm[1]}, nil}
	}
...
func (s *scanner) scanComment() *token {
	if sm := rgxComment.FindStringSubmatch(s.buffer); len(sm) != 0 {
		...
		s.consume(len(sm[0]))
		return &token{tokComment, sm[2], map[string]string{"Mode": mode}, nil}
	}
...
func (s *scanner) scanId() *token {
	if sm := rgxId.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokId, sm[1], map[string]string{"Condition": sm[2]}, nil}
	}
...
func (s *scanner) scanClassName() *token {
	if sm := rgxClassName.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokClassName, sm[1], map[string]string{"Condition": sm[2]}, nil}
	}
...
var rgxAttributeJade = regexp.MustCompile(`^([\w\-]+)\s*(=)?`)
...
func (s *scanner) scanAttribute() *token {
	if sm := rgxAttribute.FindStringSubmatch(s.buffer); len(sm) != 0 {
		...
		if len(sm[3]) != 0 || sm[2] == "" {
			return &token{tokAttribute, sm[1], map[string]string{"Content": sm[3], "Mode": "raw", "Condition": sm[5]}, nil}
		}

		return &token{tokAttribute, sm[1], map[string]string{"Content": sm[4], "Mode": "expression", "Condition": sm[5]}, nil}

	} else {
		if s.buffer[0] == '(' {
			s.consume(1)

			attributeList := make([]*token, 0)

			for {
				if sm := rgxAttributeJade.FindStringSubmatch(s.buffer); len(sm) != 0 {
					s.consume(len(sm[0]))
					...
					if isExpression {
						...
						if i > 2 {
							value = s.buffer[1 : i-1]
						} else {
							value = s.buffer[:i]
						}
						attributeList = append(attributeList, &token{tokAttribute, sm[1], map[string]string{"Content": value, "Mode": "raw", "Condition": ""}, nil})
					}

					s.consume(i)
					for s.buffer[0] == ',' || s.buffer[0] == ' ' {
						s.consume(1)
					}
			...
			return &token{tokAttributeList, "", nil, attributeList}
		}
	}
...
func (s *scanner) scanImport() *token {
	if sm := rgxImport.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokImport, sm[1], nil, nil}
	}
...
func (s *scanner) scanExtends() *token {
	if sm := rgxExtends.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokExtends, sm[1], nil, nil}
	}
...
func (s *scanner) scanBlock() *token {
	if sm := rgxBlock.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokNamedBlock, sm[2], map[string]string{"Modifier": sm[1]}, nil}
	}
...
func (s *scanner) scanTag() *token {
	if sm := rgxTag.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokTag, sm[1], nil, nil}
	}
...
func (s *scanner) scanMixin() *token {
	if sm := rgxMixin.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokMixin, sm[1], map[string]string{"Args": sm[3]}, nil}
	}
...
func (s *scanner) scanMixinCall() *token {
	if sm := rgxMixinCall.FindStringSubmatch(s.buffer); len(sm) != 0 {
		s.consume(len(sm[0]))
		return &token{tokMixinCall, sm[1], map[string]string{"Args": sm[3]}, nil}
	}
